    ...
    this.router
      ...
      .get('/', async (req: Request, res: Response, next: NextFunction) => {
        try {
          ...
          if (results.length) res.status(201).json(results[0]);
          else throw new AppErrNotFound('Customer NOT found!');
        } catch (error) {
          next(error);
        }
      })
      ...
      .post(
        ...
        async (req: IRequestExtended, res: Response, next: NextFunction) => {
          try {
            let results = <IInsertionDBResults>(
              await customerController.create(req)
            );
            res
              .status(201)
              .json({
      ...
      .put(
        ...
        async (req: IRequestExtended, res: Response, next: NextFunction) => {
          try {
            let results = <IInsertionDBResults>(
              await customerController.update(req)
            );
            ...
            if (results.affectedRows)
              res
                .status(201)
                .json({ Message: 'Customer Successfully Updated!' });
            else throw new AppErrNotFound('Customer NOT found!');
          } catch (error) {
            next(error);
          }
      ...
      .delete(
        ...
        async (req: IRequestExtended, res: Response, next: NextFunction) => {
          try {
            let results = <IInsertionDBResults>(
              await customerController.delete(req)
            );
            ...
            else return next(new AppErrNotFound('Customer NOT found!'));
          } catch (error) {
            next(error);
          }
        }
